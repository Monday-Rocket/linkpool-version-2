...
import linkpool.user.jobgroup.fixtures.createJobGroup
import linkpool.user.jobgroup.port.out.JobGroupPort
import linkpool.support.spec.afterRootTest
import linkpool.user.jobgroup.service.JobGroupQueryService

class GetJobGroupServiceTest: BehaviorSpec({
    ...
    Given("직업군이 1개 이상 등록되어 있을 경우") {
        ...
        coEvery { jobGroupPort.findAll() } returns listOf(jobGroup1, jobGroup2)
        coEvery { jobGroupPort.findById(1L) } returns jobGroup1
        coEvery { jobGroupPort.findById(3L) } returns null
